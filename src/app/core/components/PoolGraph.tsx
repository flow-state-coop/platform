...
type PoolGraphProps = {
  pool: GDAPool;
  flowStateSafeInflows: { currentFlowRate: string; sender: { id: string } }[];
  chainId: number;
  ensByAddress: {
...
export default function PoolGraph(props: PoolGraphProps) {
  const { flowStateSafeInflows, pool, chainId, ensByAddress } = props;
  ...
  const graph = useMemo(() => {
    ...
    const nodesFromFlowStateSafeDonors = flowStateSafeInflows
      .filter((inflow) => inflow.currentFlowRate !== "0")
      .map((x) => [
        {
          id: `${x.sender.id}-donor`,
          position: { x: 0, y: 0 },
          type: "custom",
          data: {
            isFlowStateSafeDonor: true,
            label:
              ensByAddress[x.sender.id]?.name ?? truncateStr(x.sender.id, 11),
            avatar: ensByAddress[x.sender.id]?.avatar,
            address: x.sender.id,
      ...
      .map((x) => [
        {
          ...
          data: {
            ...
            label:
              ...
              "0x0d9d26375b882e0ddb38a781a40e80945e3d0b9b".toLowerCase()
                ? "Flow State Safe"
    ...
    const edgesFromFlowStateSafeDonor = flowStateSafeInflows
      .map((x) => [
        {
          id: `${x.sender.id}-donor-${"0x0d9d26375b882e0ddb38a781a40e80945e3d0b9b"}-distributor`,
          source: `${x.sender.id}-donor`,
          target: `${"0x0d9d26375b882e0ddb38a781a40e80945e3d0b9b"}-distributor`,
          type: "custom",
          data: {
            flowRate: BigInt(x.currentFlowRate),
          },
