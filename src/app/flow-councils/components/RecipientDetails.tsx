...
export default function RecipientDetails(props: RecipientDetailsProps) {
  ...
  const { dispatchNewBallot, distributionPool } = useFlowCouncil();
  const { data: totalAmountReceivedByMember, dataUpdatedAt } = useReadContract({
    ...
    query: { enabled: !!distributionPool && !!recipientAddress },
  });

  const poolMember = distributionPool?.poolMembers.find(
    (m) => m.account.id === recipientAddress.toLowerCase(),
  );
          ...
          >
            ...
            <Card className="bg-transparent border-0 ms-3">
              ...
              <Card.Subtitle className="m-0">
                <Card.Link
                  href={`${superfluidExplorer}/accounts/${recipientAddress}`}
                  target="_blank"
                  className="text-info"
                >
                  {truncateStr(recipientAddress, 12)}
                </Card.Link>
              </Card.Subtitle>
            </Card>
          </Stack>
