...
export default function RecipientCard(props: RecipientProps) {
  const {
    name,
    recipientAddress,
    description,
  ...
  const recipientBallot = useMemo(
    () =>
      newBallot?.ballot.find((ballot) => ballot.recipient === recipientAddress),
    [newBallot, recipientAddress],
  );
  ...
  }, [newBallot, recipientAddress, votingPower]);
        ...
        <Card.Body className="mt-3 pb-0">
          ...
          {!!votingPower && recipientBallot && (
            ...
            >
              ...
              >
                {recipientBallot.amount === 1
                  ? `${recipientBallot.amount} vote`
                  : recipientBallot.amount > 1
                    ? `${recipientBallot.amount} votes`
                    : null}
              </Card.Text>
            </Stack>
          )}
