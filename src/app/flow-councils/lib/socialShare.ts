...
export function getVoteSocialShare({
  ...
  return {
    twitter:
      ...
      `these%20public%20goods%20builders%20at%20${encodeURIComponent(councilUiLink)}`,
    farcaster:
      "https://farcaster.xyz/~/compose?text=I+just+voted+in+the+" +
      `${councilName}+on+Flow+State.%0AJoin+me+in+supporting+these+public+` +
      "goods+builders+at" +
      `&embeds[]=${encodeURIComponent(councilUiLink)}`,
    lens:
      `https://hey.xyz/?text=I+just+voted+in+the+${councilName}+on+Flow+` +
      "State.%0AJoin+me+in+supporting+these+public+goods+builders+at+" +
