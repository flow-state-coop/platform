...
export default function Recipient(props: RecipientProps) {
  const { chainId, flowCouncilId, csfrToken } = props;
  ...
  const flowCouncilToken = network?.tokens.find(
      ...
      profiles?.map((profile: Project) => {
        ...
        for (const application of applications) {
          if (
            ...
            recipientStatus = application.status;
          }
        }
  ...
  if (
    ...
    return <span className="m-auto fs-4 fw-bold">No Flow Council found</span>;
  }
  ...
  return (
    ...
    >
      <h1 className="mt-5 mb-0">{flowCouncilMetadata.name}</h1>
      <Stack
      ...
      >
        Distributing{" "}
        {!!flowCouncilToken.icon && (
          <Image src={flowCouncilToken.icon} alt="" width={18} height={18} />
        )}
