...
// application that is still outstanding (not removed or rejected). "Admin" means
// the wallet was explicitly added as a round admin.
  ...
  try {
    ...
    const [application, admin] = await Promise.all([
      db
        ...
        .where("applications.status", "not in", ["REMOVED", "REJECTED"])
        .executeTakeFirst(),
      db
