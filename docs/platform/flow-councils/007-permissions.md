...
- **Voter Review** (`VOTER_MANAGER_ROLE`) – manages Council membership and [voter groups](002-council-membership.md).
- **Recipient Review** (`RECIPIENT_MANAGER_ROLE`) – reviews [applications](006-applications.md) and manages recipients in the distribution pool.
...
For councils with a GoodDollar voter group, a Flow State–sponsored bot is granted the **Voter Review** role so it can add verified voters automatically. Revoking that role from the bot is the kill switch for self-claim. See [Council Membership](002-council-membership.md) for the full flow.
