...
Programmatic writes are unlocked per pool with a **one-time payment of 10 USDC**, sent on the pool's chain to the Flow State bot's address. The Flow Splitter API is in beta, and the payment unlocks writes for the pool through the beta period; if pricing changes at graduation, beta payments count toward the new model. The card's first step leads with **Pay 10 USDC to unlock** and shows a green check once the payment is counted. Reads, key minting, and job polls work without paying, so an integration can be built and tested end to end first; a write on a locked pool is refused with a clear error rather than accepted and dropped. OP Sepolia pools are not gated, so a testnet integration costs nothing.

Pay from the wallet you signed in with. A payment is claimed for the pool by the wallet that sent it, or by the wallet the USDC left when a contract wallet such as a Safe pays through an executor, which is what stops anyone else claiming yours. If verification is interrupted after the transfer went out, the step swaps the pay button for a refresh icon and says the payment isn't lost; the refresh retries the check, and the payment is found and counted rather than charged twice. If the browser that paid lost track of the transaction, **Already paid? Verify the transaction** takes the transaction hash, or a block explorer link to it, and verifies the same payment by hand.
