...
- **`votes`** — array of 1–1000 entries. Order is not significant.
...
| `200` | `{ "success": true, "skipped": true }` | Computed ballot matches the bot's current on-chain ballot — no transaction sent. |
...
- Not stored in plaintext — only the SHA-256 hash is persisted. The token is shown once in the Metrics API panel on creation.
